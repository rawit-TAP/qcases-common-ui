...
> = ({ children, options, sessionChanged }) => {
  const checkSessionIntervalInMs = !options.checkSessionInterval
    ? 5 * 1000
  ...
  useEffect(() => {
    const checkSession = () => {
      ...
      const win = (
        document.getElementById(sessionCheckIframeId) as HTMLIFrameElement
      ).contentWindow;

      console.log("Checking session id", options.getSessionIdFn());
      win!.postMessage(options.getSessionIdFn(), "*");
    };
    checkSession();
  ...
  return (
    <Fragment>
      <iframe
        src={options.idpOrigin + options.checkSessionIframePath}
        id={sessionCheckIframeId}
        style={{
          display: "none",
        }}
      ></iframe>
